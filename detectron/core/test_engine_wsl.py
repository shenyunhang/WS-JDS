...
from detectron.core.test_wsl import im_detect_all
...
def multi_gpu_test_net_on_dataset(
    ...
    binary = os.path.join(binary_dir, 'test_net_wsl' + binary_ext)
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        with c2_utils.NamedCudaScope(gpu_id):
            cls_boxes_i, cls_segms_i, cls_keyps_i, masks_i = im_detect_all(
        ...
        if cfg.VIS:
            ...
            if masks_i is not None:
                ones = np.ones((1, masks_i.shape[1]))
                cpg_vis.save_common(masks_i,
                                      ones,
                                      '{:d}_{:s}'.format(i, im_name),
                                      '',
                                      os.path.join(output_dir, 'vis'))
