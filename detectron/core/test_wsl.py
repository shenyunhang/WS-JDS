...
def im_detect_all(model, im, box_proposals, obn_scores, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        scores, boxes, im_scale = im_detect_bbox_aug(model, im, box_proposals, obn_scores)
    else:
...
def im_detect_bbox_aug(model, im, box_proposals=None, obn_scores=None):
    ...
    def add_preds_t(scores_t, boxes_t):
        ...
        if cfg.TEST.BBOX_AUG.COORD_HEUR == 'ID':
            assert np.array_equal(
                boxes_ts[0],
                boxes_t), 'boxes at each scale should be the same'
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        scores_scl, boxes_scl = im_detect_bbox_scale(
            model, im, scale, max_size, box_proposals, obn_scores
        )
        add_preds_t(scores_scl, boxes_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            scores_scl_hf, boxes_scl_hf = im_detect_bbox_scale(
                model, im, scale, max_size, box_proposals, obn_scores, hflip=True
            )
...
def im_detect_bbox_hflip(
    model, im, target_scale, target_max_size, box_proposals=None, obn_scores=None
):
    ...
    scores_hf, boxes_hf, im_scale = im_detect_bbox(
        model, im_hf, target_scale, target_max_size, boxes=box_proposals_hf, obn_scores=obn_scores
    )
...
def im_detect_bbox_scale(
    model, im, target_scale, target_max_size, box_proposals=None, obn_scores=None, hflip=False
):
    ...
    if hflip:
        scores_scl, boxes_scl, _ = im_detect_bbox_hflip(
            model, im, target_scale, target_max_size, box_proposals=box_proposals, obn_scores=obn_scores
        )
    else:
        scores_scl, boxes_scl, _ = im_detect_bbox(
            model, im, target_scale, target_max_size, boxes=box_proposals, obn_scores=obn_scores
        )
    return scores_scl, boxes_scl


def im_detect_bbox_aspect_ratio(
    model, im, aspect_ratio, box_proposals=None, obn_scores=None, hflip=False
):
...
def im_detect_mask_deeplab(model, im_scale, boxes):
    ...
    pred_masks = workspace.FetchBlob(
        core.ScopedName('mask_fc8_bgfg')
    )
    # .squeeze()
    print('mask bgfg: ', pred_masks.shape, pred_masks.max(), pred_masks.min(), pred_masks.mean())
    ...
    pred_masks_crf = workspace.FetchBlob(
        core.ScopedName('mask_fc8_crf')
    )
    # .squeeze()
    print('mask_fc8_crf: ', pred_masks_crf.shape, pred_masks_crf.max(), pred_masks_crf.min(), pred_masks_crf.mean())
