...
def get_mAP():
    ...
    for line in log_file.readlines():
        line = line.strip()
        ma = re.search('Mean AP = [0-9]+([.][0-9]+)?', line)
        if ma is None:
            continue
        # print(ma)
        # print(ma.group())
...
def draw():
    ...
    major_ticks = np.arange(0.2, 0.6, 0.05)
    ...
    ax1.set_xlim([0, mAP_epoch_values[-1]])
    ax1.set_ylim([0, 2])
    plt.savefig(os.path.join(output_dir, uuid + '_plot_2.png'), dpi=1200)

    # set the limits
    ax1.set_xlim([0, mAP_epoch_values[-1]])
    ax1.set_ylim([0, 1])
    plt.savefig(os.path.join(output_dir, uuid + '_plot_1.png'), dpi=1200)
